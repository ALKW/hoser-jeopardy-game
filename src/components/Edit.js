...
class Edit extends Component {
  constructor(props) {
    ...
    ipcRenderer.on('open-file-reply', (event, fileData) => {
      var fileContentsPromise;
      if (getExtension(fileData.name) === "csv") {
        fileContentsPromise = parseCsv(fileData.fileContents);
      } else {
        fileContentsPromise = Promise.resolve(fileData.fileContents);
      }
      fileContentsPromise.then(contents => {
        try {
